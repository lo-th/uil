...
export class Renderer  {
   ...
   constructor( gl ) {
       ...
       * @private
       */
      this.vertexShader_ = this.compileShader_( vertexShaderSource_, gl.VERTEX_SHADER);

      /**
       * @type {WebGLShader}
       * @private
       */
      this.fragmentShader_ = this.compileShader_( fragmentShaderSource_, gl.FRAGMENT_SHADER);
   ...
   createDxtTexture (dxtData, width, height, format) {
      var gl = this.gl_;
   ...
   createCompressedTexture (data, width, height, format) {
      var gl = this.gl_;
      var tex = gl.createTexture();
      gl.bindTexture(gl.TEXTURE_2D, tex);
      gl.compressedTexImage2D(
   ...
   createHalfRGBATexture (data, width, height, format) {
      var gl = this.gl_;
      var tex = gl.createTexture();
      gl.bindTexture(gl.TEXTURE_2D, tex);
      gl.texImage2D(
   ...
   createRgb565Texture (rgb565Data, width, height) {
      var gl = this.gl_;
      var tex = gl.createTexture();
      gl.bindTexture(gl.TEXTURE_2D, tex);
   ...
   createRgbaTexture (rgbaData, width, height) {
      var gl = this.gl_;
      var tex = gl.createTexture();
      gl.bindTexture(gl.TEXTURE_2D, tex);
      gl.texImage2D(
   ...
   drawTexture (texture, width, height, mode, scale, linearToSRGBFlag) {
      var gl = this.gl_;
      // draw scene
    ...
    * @private
    */
   compileShader_ (shaderSource, type) {
      var gl = this.gl_;
