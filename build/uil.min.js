'use strict';var Crea=Crea||function(){var a=document,b=a.getElementsByTagName("head")[0],c="height width top left bottom right margin-left margin-right margin-top margin-bottom".split(" "),d=["rect","circle","path","polygon","text","pattern","defs","g","transform",,"line","foreignObject","linearGradient","stop","animate","radialGradient"];Crea=function(){};Crea.setSvg=function(a,b,c,d){-1===d?a.setAttributeNS(null,b,c):a.childNodes[d||0].setAttributeNS(null,b,c)};Crea.setDom=function(a,b,d){var g=
-1!==c.indexOf(b)?"px":"";a.style[b]=d+g};Crea.clearDom=function(a){for(;a.children.length;){if(a.lastChild.children)for(;a.lastChild.children.length;)a.lastChild.removeChild(a.lastChild.lastChild);a.removeChild(a.lastChild)}};Crea.dom=function(b,c,f,g,h,l){c=c||"div";-1!==d.indexOf(c)?(void 0===h&&(h=a.createElementNS("http://www.w3.org/2000/svg","svg")),Crea.add(h,c,g,l)):void 0===h&&(h=a.createElementNS("http://www.w3.org/1999/xhtml",c));b&&h.setAttribute("class",b);f&&(h.style.cssText=f);return void 0===
l?h:h.childNodes[l||0]};Crea.cc=function(c,d,f){f=void 0===f?".":"";"*"===c&&(f="");var g=a.createElement("style");g.setAttribute("type","text/css");g.setAttribute("id",c);b.appendChild(g);(g.sheet||{}).insertRule?g.sheet.insertRule(f+c+"{"+d+"}",0):(g.styleSheet||g.sheet).addRule(f+c,d)};Crea.add=function(a,b,c,d){b=document.createElementNS("http://www.w3.org/2000/svg",b);this.set(b,c);this.get(a,d).appendChild(b);return b};Crea.set=function(a,b){for(var c in b)"txt"===c&&(a.textContent=b[c]),a.setAttributeNS(null,
c,b[c]);a.style.pointerEvents="none"};Crea.get=function(a,b){if(void 0===b)return a;if(!isNaN(b))return a.childNodes[b];if(b instanceof Array){if(2===b.length)return a.childNodes[b[0]].childNodes[b[1]];if(3===b.length)return a.childNodes[b[0]].childNodes[b[1]].childNodes[b[2]]}};return Crea}();var define,module,exports,UIL=UIL||function(){return{main:null,REVISION:"0.9",DEF:!1,WIDTH:300,BW:190,AW:100,UNS:"-o-user-select:none; -ms-user-select:none; -khtml-user-select:none; -webkit-user-select:none; -moz-user-select:none;",US:"-o-user-select:text; -ms-user-select:text; -khtml-user-select:text; -webkit-user-select:text; -moz-user-select:text;",TXT:'font-family:"Lucida Console", Monaco, monospace; font-size:11px; color:#cccccc; background:none; padding:3px 10px; left:0; top:0; height:17px; width:100px; overflow:hidden; white-space: nowrap;',
DOM:Crea.dom,CC:Crea.cc,setSvg:Crea.setSvg,COLOR:"N",SELECT:"#035fcf",MOVING:"#03afff",SELECTDOWN:"#024699",SVGB:"rgba(0,0,0,0.2)",SVGC:"rgba(120,120,120,0.6)",BorderSelect:"rgba(3,95,207,0.6)",sizer:function(a){this.WIDTH=~~a;a=this.WIDTH/3;this.BW=~~(2*a-10);this.AW=~~a;this.main&&this.main.changeWidth()},classDefine:function(){UIL.CC("UIL",UIL.UNS+" position:absolute; pointer-events:none; box-sizing:border-box; margin:0; padding:0; border:none; ");UIL.CC("UIL.content"," display:block; width:300px; height:auto; top:0; left:0; overflow:hidden; background:none; transition:height, 0.1s ease-out;");
...
this.c[0]=null;this.callback&&(this.callback=null);this.value&&(this.value=null)},clearDOM:function(a){for(;a.children.length;){if(a.lastChild.children)for(;a.lastChild.children.length;)a.lastChild.removeChild(a.lastChild.lastChild);a.removeChild(a.lastChild)}},setTypeNumber:function(a){this.min=void 0===a.min?-Infinity:a.min;this.max=void 0===a.max?Infinity:a.max;this.precision=void 0===a.precision?2:a.precision;var b;switch(this.precision){case 0:b=1;break;case 1:b=0.1;break;case 2:b=0.01;break;
case 3:b=0.001;break;case 4:b=1E-4}this.step=void 0===a.step?b:a.step},numValue:function(a){return 1*Math.min(this.max,Math.max(this.min,a)).toFixed(this.precision)},rSize:function(){"circular"!==this.type&&"knob"!==this.type&&(this.c[0].style.width=this.size+"px",this.simple||(this.c[1].style.width=this.sa+"px"))},addEvent:function(){for(var a=this.c.length,b,c;a--;)if(c=this.c[a],void 0!==c&&void 0!==c.events)for(b=c.events.length;b--;)c.addEventListener(c.events[b],this,!1)},clearEvent:function(){for(var a=
...
UIL.Color.prototype.drawCircle=function(){var a=this.radius,b=this.wheelWidth,c=8/a/24*Math.PI,d=this.ctxMask,e=0,k,f,g,h;d.save();d.lineWidth=b/a;d.scale(a,a);for(var l=0;24>=l;++l)h=l/24,a=h*Math.PI*2,b=[Math.sin(e),-Math.cos(e),Math.sin(a),-Math.cos(a)],f=0.5*(e+a),g=1/Math.cos(0.5*(a-e)),e=Math.sin(f)*g,f=-Math.cos(f)*g,h=UIL.pack(UIL.HSLToRGB([h,1,0.5])),0<l&&(g=d.createLinearGradient(b[0],b[1],b[2],b[3]),g.addColorStop(0,k),g.addColorStop(1,h),d.strokeStyle=g,d.beginPath(),d.moveTo(b[0],b[1]),
d.quadraticCurveTo(e,f,b[2],b[3]),d.stroke()),e=a-c,k=h;d.restore()};
UIL.Color.prototype.drawMarkers=function(){var a=this.markerSize,b=this.radius,c=this.width,d=Math.ceil(a/4),e=a-d+1,k=this.invert?"#fff":"#000",f=this.invert?"#000":"#fff",g=6.28*this.hsl[0],b=[Math.sin(g)*b,-Math.cos(g)*b,2*this.square*(0.5-this.hsl[1]),2*this.square*(0.5-this.hsl[2])],a=[{x:b[2],y:b[3],r:a,c:k,lw:d},{x:b[2],y:b[3],r:e,c:f,lw:d+1},{x:b[0],y:b[1],r:a,c:"#fff",lw:d},{x:b[0],y:b[1],r:e,c:"#000",lw:d+1}];this.ctxOverlay.clearRect(-this.mid,-this.mid,c,c);for(c=a.length;c--;)d=a[c],
...
UIL.RGBtoHSL=function(a){var b=a[0],c=a[1];a=a[2];var d=Math.min(b,c,a),e=Math.max(b,c,a),k=e-d,f=0,g=0,d=(d+e)/2;0<d&&1>d&&(g=k/(0.5>d?2*d:2-2*d));0<k&&(e==b&&e!=c&&(f+=(c-a)/k),e==c&&e!=a&&(f+=2+(a-b)/k),e==a&&e!=b&&(f+=4+(b-c)/k),f/=6);return[f,g,d]};UIL.Slide=function(a){UIL.Proto.call(this,a);this.type="slide";this.setTypeNumber(a);this.range=this.max-this.min;this.width=UIL.BW-40;this.w=this.width-8;this.h=a.height||20;this.h=11>this.h?11:this.h;this.old=this.value=a.value||0;this.isOver=this.isDown=!1;a=0.5*a.height-10;void 0!==this.c[1]&&(this.c[1].style.top=a+"px");this.c[2]=UIL.DOM("UIL text","div","top:"+a+"px; text-align:right; width:40px; padding:3px 5px; color:"+this.fontColor);this.c[3]=UIL.DOM("UIL svgbox","rect","width:"+this.width+
...
y2:10,stroke:UIL.SVGC,"stroke-width":1},this.c[3].childNodes[1],0);UIL.DOM(null,"line",null,{x1:10,x2:5,y1:0,y2:10,stroke:UIL.SVGC,"stroke-width":1},this.c[3].childNodes[1],0);this.c[3].events=["mouseover","mousedown","mouseout"];this.init()};UIL.Slide.prototype=Object.create(UIL.Proto.prototype);UIL.Slide.prototype.constructor=UIL.Slide;
...
UIL.Circular.prototype.up=function(a){this.isDown=!1;document.removeEventListener("mouseup",this,!1);document.removeEventListener("mousemove",this,!1);this.isOver?this.mode(1):this.mode(0)};UIL.Circular.prototype.down=function(a){this.isDown=!0;this.oldr=null;this.move(a);this.mode(2);document.addEventListener("mouseup",this,!1);document.addEventListener("mousemove",this,!1)};
...
UIL.Circular.prototype.makePath=function(){var a=this.radius,b=this.percent*this.twoPi-0.001,c=a+a*Math.sin(0),d=a-a*Math.cos(0),e=a+a*Math.sin(b),k=a-a*Math.cos(b);return"M "+a+","+a+" L "+c+","+d+" A "+a+","+a+" 0 "+(b-0>Math.PI?1:0)+" 1 "+e+","+k+" Z"};UIL.Circular.prototype.update=function(a){this.c[2].textContent=this.value;this.percent=(this.value-this.min)/this.range;UIL.setSvg(this.c[4],"d",this.makePath());a&&this.callback(this.value)};UIL.Knob=function(a){UIL.Proto.call(this,a);this.type="knob";this.setTypeNumber(a);this.range=this.max-this.min;this.mPI=0.8*Math.PI;this.toDeg=180/Math.PI;this.cirRange=2*this.mPI;this.value=a.value||0;this.radius=a.radius||15;this.size=2*this.radius+20;void 0!==a.size&&(this.size=a.size,this.radius=0.5*~~(this.size-20));this.w=2*this.radius;this.height=2*this.radius;this.h=a.height||this.height+40;this.top=0;this.c[0].style.width=this.size+"px";void 0!==this.c[1]&&(this.c[1].style.width=this.size+
...
UIL.Knob.prototype.move=function(a){if(this.isDown){a.preventDefault();var b=this.c[3].getBoundingClientRect();this.r=-1*Math.atan2(this.radius-(a.clientX-b.left),this.radius-(a.clientY-b.top));this.r>this.mPI&&(this.r=this.mPI);this.r<-this.mPI&&(this.r=-this.mPI);null!==this.oldr&&(this.r=Math.abs(this.r-this.oldr)>Math.PI?this.oldr:this.r);this.value=this.numValue(1/this.cirRange*this.range*(this.r+this.mPI)+this.min);this.update(!0);this.oldr=this.r}};
UIL.Knob.prototype.makeGrad=function(){for(var a="",b=Math.PI+this.mPI,c=(b-(Math.PI-this.mPI))/this.radius,d=this.radius,e,k,f,g,h=0;h<=this.radius;++h)e=b-c*h,k=d+Math.sin(e)*d,f=d+Math.cos(e)*d,g=d+Math.sin(e)*(d-3),e=d+Math.cos(e)*(d-3),a+="M"+k+" "+f+" L"+g+" "+e+" ";return a};
UIL.Knob.prototype.update=function(a){this.c[2].textContent=this.value;this.percent=(this.value-this.min)/this.range;this.rr=(this.percent*this.cirRange-this.mPI)*this.toDeg;UIL.setSvg(this.c[4],"transform","rotate("+this.rr+" "+this.radius+" "+this.radius+")");a&&this.callback(this.value)};
