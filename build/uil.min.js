...
var c;switch(a){case "button":c=new UIL.Button(b);break;case "string":c=new UIL.String(b);break;case "number":c=new UIL.Number(b);break;case "title":c=new UIL.Title(b);break;case "color":c=new UIL.Color(b);break;case "slide":c=new UIL.Slide(b);break;case "bool":c=new UIL.Bool(b);break;case "list":c=new UIL.List(b);break;case "group":c=new UIL.Group(b);break;case "knob":c=new UIL.Knob(b);break;case "circular":c=new UIL.Circular(b);break;case "joystick":c=new UIL.Joystick(b)}this.uis.push(c);this.calc();
return c},resize:function(a){this.calc();this.testHeight()},remove:function(a){a=this.uis.indexOf(a);-1!==a&&(this.uis[a].clear(),this.uis.splice(a,1))},clear:function(){this.update(0);for(var a=this.uis.length;a--;)this.uis[a].clear(),this.uis[a]=null,this.uis.pop();this.uis=[];this.calc()},showScroll:function(a){this.isScroll=!0;this.total=this.height-20;this.maxView=this.maxHeight-20;this.ratio=this.maxView/this.total;this.sh=this.maxView*this.ratio;20>this.sh&&(this.sh=20);this.range=this.maxView-
this.sh;this.scrollBG.style.display="block";this.scrollBG.style.height=this.maxView+"px";this.scroll.style.height=this.sh+"px";this.update(0)},hideScroll:function(){this.isScroll=!1;this.update(0);this.scrollBG.style.display="none"},calc:function(a){void 0!==a?(this.height+=a,this.content.style.height=this.height+"px"):(this.height=this.inner.offsetHeight+20,this.content.style.height=this.height+"px","close"!==this.bottom.textContent&&(this.bottom.textContent="close"));this.tmp=setTimeout(this.testHeight.bind(this),
...
this.oldr=this.r)};UIL.Knob.prototype.makeGrad=function(){for(var a="",b=Math.PI+this.mPI,c=(b-(Math.PI-this.mPI))/this.radius,d=this.radius,e,k,f,g,h=0;h<=this.radius;++h)e=b-c*h,k=d+Math.sin(e)*d,f=d+Math.cos(e)*d,g=d+Math.sin(e)*(d-3),e=d+Math.cos(e)*(d-3),a+="M"+k+" "+f+" L"+g+" "+e+" ";return a};
UIL.Knob.prototype.update=function(a){this.c[2].textContent=this.value;this.percent=(this.value-this.min)/this.range;this.rr=(this.percent*this.cirRange-this.mPI)*this.toDeg;UIL.setSvg(this.c[4],"transform","rotate("+this.rr+" "+this.radius+" "+this.radius+")");a&&this.callback(this.value)};UIL.Joystick=function(a){UIL.Proto.call(this,a);this.type="joystick";this.value=[0,0];this.precision=a.precision||2;this.multiplicator=a.multiplicator||1;this.oldy=this.oldx=this.y=this.x=0;this.interval=null;this.radius=a.radius||50;this.size=2*this.radius+20;void 0!==a.size&&(this.size=a.size,this.radius=0.5*~~(this.size-20));this.innerRadius=a.innerRadius||0.6*this.radius;this.maxDistance=this.radius-this.innerRadius-5;this.height=2*this.radius;this.h=a.height||this.height+40;this.top=0;this.c[0].style.width=
...
style:"stop-color:rgb(0,65,167); stop-opacity:1;"},this.c[4].childNodes[1],1);this.c[5].textContent="x"+this.value[0]+" y"+this.value[1];this.c[2].events=["mouseover","mousedown","mouseout"];this.init();this.update()};UIL.Joystick.prototype=Object.create(UIL.Proto.prototype);UIL.Joystick.prototype.constructor=UIL.Joystick;
...
UIL.Joystick.prototype.update=function(){null!==this.interval&&(this.isDown||(this.x+=(0-this.x)/3,this.y+=(0-this.y)/3),this.x.toFixed(2)===this.oldx.toFixed(2)&&this.y.toFixed(2)===this.oldy.toFixed(2)&&(this.y=this.x=0));var a=this.radius-this.x*this.maxDistance,b=this.radius-this.y*this.maxDistance,c=a+5*(1-this.x)+5,d=b+5*(1-this.y)+10;this.value[0]=1*-(this.x*this.multiplicator).toFixed(this.precision);this.value[1]=1*(this.y*this.multiplicator).toFixed(this.precision);this.c[5].textContent=
"x"+this.value[0]+" y"+this.value[1];UIL.setSvg(this.c[3],"cx",c);UIL.setSvg(this.c[3],"cy",d);UIL.setSvg(this.c[4],"cx",a);UIL.setSvg(this.c[4],"cy",b);this.oldx=this.x;this.oldy=this.y;this.callback(this.value);null!==this.interval&&0===this.x&&0===this.y&&(clearInterval(this.interval),this.interval=null)};
