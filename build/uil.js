...
(function (global, factory) {
	...
	class Slide extends Proto {
		...
		update(up) {
			...
			let uneased = this.easing == 1 ? normalized : Math.pow(normalized, 1 / this.easing);
	...
	class TreeList extends Proto {
		constructor(o = {}) {
			...
			this.lastLeaf = {
				parentPath: [],
				key: null
			}; // parentPath es la ruta hasta el mapa padre
		...
		getNodeAtPath(path) {
			...
			for (let i = 0; i < path.length; i++) {
				if (TreeList.isMap(node)) {
					if (!Object.prototype.hasOwnProperty.call(node, path[i])) return {
						node: null,
						depth: i
					};
					node = node[path[i]];
			...
			for (let L = 0; L < this.levels.length; L++) {
				...
				if (lvl.type === 'map') {
					...
					for (let i = 0; i < nRows; i++) {
						...
						const itz = isReal ? lvl.items[i].zone : {
							x: z.x,
							y: z.y + i * this.lineH,
							w: z.w,
							h: this.lineH
