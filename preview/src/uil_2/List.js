UIL.List = function( o ){
    ...
    for( var i=0; i<this.length; i++ ){
        n = this.list[i];
        item = UIL.DOM('UIL text', 'div', 'position:relative; margin-bottom:1px; pointer-events:auto; cursor:pointer; width:'+this.w+'px; height:'+this.itemHeight+'px; line-height:'+(this.itemHeight-5)+'px;');
...
UIL.List.prototype.handleEvent = function( e ) {
    ...
    switch( e.type ) {
        ...
        case 'mouseover': if(name === 'title')this.mode(1); else if(name === 'item') this.overItem(e); break;
...
*/
UIL.List.prototype.overItem = function(e){

    console.log()
    var i = this.listIn.childNodes.length;
    while(i--){
        this.listIn.childNodes[i].style.background = 'rgba(0,0,0,0.2)';
        this.listIn.childNodes[i].style.color = this.fontColor;
    }
    e.target.style.background = UIL.SELECT;
    e.target.style.color = '#FFF';
