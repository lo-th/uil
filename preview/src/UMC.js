...
var UMC = UMC || ( function () {
    ...
    UMC.toCanvas = function( canvas, content, w, h ){
        ...
        if( typeof content === 'string' ){
            ...
            //dcopy = document.implementation.createDocument('http://www.w3.org/1999/xhtml', 'html', null);
            dcopy = document.createElement("body");
            dcopy.setAttribute( 'xmlns', "http://www.w3.org/1999/xhtml" );
            //dcopy.xmlns = "http://www.w3.org/1999/xhtml";
            //dcopy.documentElement.appendChild(body);
            //dcopy.firstChild.appendChild(body);


            var frame = UMC.dom( null, 'iframe', 'position:abolute; left:0; top:0; width:'+w+'px; height:'+h+'px;' );
            frame.src = content;

            dcopy.appendChild(frame);
       ...
       // var dcopy = UMC.dom(null, 'div', 'width:'+w+'px; height:'+h+'px; top:0; left:0;' );
        ...
        //var tmp_svg = UMC.dom( null, 'foreignObject', 'position:abolute; left:0; top:0; ', { width:w, height:h } );
        var tmp_svg = UMC.dom( null, 'foreignObject', 'position:abolute; left:0; top:0; width:'+w+'; height:'+h+';', { width:w, height:h } );
        ...
        /*if( tmp_svg === null ){
            ...
            tmp_svg = UMC.dom( null, 'foreignObject', 'position:abolute; left:0; top:0;', { width:w, height:h } );
            tmp_svg.childNodes[0].appendChild( dcopy );

            tmp_svg.setAttribute("version", "1.1");
            tmp_svg.setAttribute('xmlns', 'http://www.w3.org/2000/svg' );
            tmp_svg.setAttribute("xmlns:xlink", "http://www.w3.org/1999/xlink");

            tmp_svg.setAttribute('width', w );
            tmp_svg.setAttribute('height', h );
            tmp_svg.childNodes[0].setAttribute('width', '100%' );
            tmp_svg.childNodes[0].setAttribute('height', '100%' );

        } else {
            
            tmp_svg.childNodes[0].removeChild( tmp_svg.childNodes[0].firstChild );
            tmp_svg.setAttribute('width', w );
            tmp_svg.setAttribute('height', h );
            tmp_svg.childNodes[0].appendChild( dcopy );
       ...
       // var data = 'data:image/svg+xml;base64,'+ window.btoa(( new XMLSerializer ).serializeToString( tmp_svg ));
       // var data = 'data:image/svg+xml;charset=utf-8,'+ window.btoa(( new XMLSerializer ).serializeToString( tmp_svg ));
       ...
       // console.timeEnd("start copy");
        ...
        img.src = 'data:image/svg+xml;base64,'+ window.btoa(( new XMLSerializer ).serializeToString( tmp_svg ));
       ...
       // tmp = setTimeout(function() {
        ...
        /*var img = new Image();
        var DOMURL = self.URL || self.webkitURL || self;
        var data = ( new XMLSerializer ).serializeToString( tmp_svg );
